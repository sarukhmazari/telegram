...
export const authMiddleware = async (ctx, next) => {
    ...
    try {
        const dbUser = await UserService.findOrCreateUser(ctx.from.id, ctx.from.username, ctx.from.first_name, ctx.from.last_name, referralCode);
        if (dbUser.isBanned) {
...
export const adminGuard = async (ctx, next) => {
    ...
    const isCbDataAdmin = Boolean(ctx.callbackQuery &&
