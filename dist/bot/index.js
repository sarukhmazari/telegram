...
import { getCategoriesKeyboard } from './keyboards/store.js';
...
// /start command
bot.start(async (ctx) => {
    const welcomeText = `👋 *Welcome to ${config.STORE_NAME}!*\n\n` +
...
bot.action('menu_store', async (ctx) => {
    ...
    await ctx.editMessageText('🛍 *Select a Product Category:*', {
        ...
        if (!String(err).includes('message is not modified'))
            logger.warn('menu_store edit error', { err });
    });
});
// Main menu callback
bot.action('menu_main', async (ctx) => {
    await ctx.editMessageText(`🏠 *Main Menu*\n\nWelcome back to *${config.STORE_NAME}*!`, {
        ...
        if (!String(err).includes('message is not modified'))
            logger.warn('menu_main edit error', { err });
    });
});
// Balance Menu
bot.action('menu_balance', async (ctx) => {
    ...
    await ctx.editMessageText(msg, {
        ...
        if (!String(err).includes('message is not modified'))
            logger.warn('menu_balance edit error', { err });
    });
});
// Deposit / Top-up Balance Action
bot.action('menu_deposit', async (ctx) => {
    const msg = `💳 *Wallet Balance Top-up*\n\n` +
    ...
    await ctx.editMessageText(msg, {
        ...
        if (!String(err).includes('message is not modified'))
            logger.warn('menu_deposit edit error', { err });
    });
});
// Account Menu
bot.action('menu_account', async (ctx) => {
    ...
    await ctx.editMessageText(msg, {
        ...
        if (!String(err).includes('message is not modified'))
            logger.warn('menu_account edit error', { err });
    });
});
// My Orders Menu
bot.action('menu_orders', async (ctx) => {
    ...
    if (orders.length === 0) {
        await ctx.editMessageText('📦 *Your Order History*\n\nYou have no past orders yet.', {
            ...
            if (!String(err).includes('message is not modified'))
                logger.warn('menu_orders edit error', { err });
        });
        return;
    }
...
bot.action('menu_promotions', async (ctx) => {
    ...
    await ctx.editMessageText(msg, {
        ...
        if (!String(err).includes('message is not modified'))
            logger.warn('menu_promotions edit error', { err });
    });
});
// Support Menu
bot.action('menu_support', async (ctx) => {
    ...
    await ctx.editMessageText(helpText, {
        ...
        if (!String(err).includes('message is not modified'))
            logger.warn('menu_support edit error', { err });
    });
});
...
bot.action(/^pay_method_jazzcash_(.+)$/, async (ctx) => {
    ...
    const msg = `💳 *JazzCash Payment Instructions*\n\n` +
...
bot.action(/^pay_method_wallet_(.+)$/, async (ctx) => {
    ...
    try {
        const result = await walletProvider.createPayment(order.id, user.id, Number(order.totalAmount), order.currency, idempotencyKey);
...
bot.on(['text', 'photo'], async (ctx, next) => {
    if (ctx.session?.userState === 'AWAITING_PAYMENT_PROOF' && ctx.session?.userData?.orderId) {
        ...
        const paymentResult = await manualProvider.createPayment(orderId, user.id, parseFloat(amount), 'USD', idempotencyKey, { transactionReference: trxRef, proofFileId });
        ctx.session.userState = undefined;
        ctx.session.userData = undefined;
        // Send confirmation to customer
        await ctx.reply(`✅ *Payment Proof Submitted Successfully!*\n\n` +
        ...
        const adminMsg = `💳 *New Payment Proof Received!*\n\n` +
            `• *Order Number:* \`#${orderNumber}\` \n` +
