...
import { getAdminMainKeyboard, getPaymentReviewKeyboard, getAdminProductsKeyboard, getAdminCategoriesKeyboard, } from '../keyboards/admin.js';
...
adminComposer.action('admin_dashboard', async (ctx) => {
    ...
    const msg = `📊 *Store Analytics & Dashboard*\n\n` +
...
adminComposer.action('admin_payments', async (ctx) => {
    ...
    const msg = `💳 *Pending Payment Review* (1 of ${pendingPayments.length})\n\n` +
...
adminComposer.action(/^admin_approve_pay_(.+)$/, async (ctx) => {
    ...
    try {
        const success = await AdminService.approvePayment(paymentId, 'Approved by Admin', ctx);
        if (success) {
...
adminComposer.action(/^admin_reject_pay_(.+)$/, async (ctx) => {
    ...
    try {
        await AdminService.rejectPayment(paymentId, 'Rejected by Admin', ctx);
...
adminComposer.action('admin_users', async (ctx) => {
    ...
    const msg = `👥 *Customer & User Management*\n\n` +
...
adminComposer.action('admin_broadcast', async (ctx) => {
    ...
    const msg = `📢 *Customer Mass Broadcast*\n\n` +
...
adminComposer.action('admin_confirm_broadcast', async (ctx) => {
    ...
    const broadcast = await BroadcastService.sendBroadcast(ctx.from.id.toString(), broadcastText || '', ctx, fileId);
    const confirmMsg = `🎉 *Broadcast Dispatched Successfully!*\n\n` +
...
adminComposer.action(/^admin_add_stock_(.+)$/, async (ctx) => {
    ...
    const promptText = `📥 *Upload Accounts / Stock for ${variant.product.name}*\n\n` +
...
adminComposer.action(/^admin_assign_stock_(.+)$/, async (ctx) => {
    ...
    const summaryMsg = `🎉 *Accounts Successfully Uploaded & Encrypted!*\n\n` +
...
adminComposer.on(['text', 'photo'], async (ctx, next) => {
    ...
    if (state === 'AWAITING_BROADCAST_MESSAGE') {
        ...
        const previewText = `📢 *Broadcast Preview & Confirmation*\n\n` +
    ...
    if (isAccountFormat && ctx.isAdmin) {
        ...
        if (lines.length > 0) {
            ...
            if (variants.length > 0) {
                ...
                await ctx.reply(`📥 *Detected ${lines.length} account line(s)!*\n\nSelect which product to upload these accounts to:`, {
                    parse_mode: 'Markdown',
                    reply_markup: Markup.inlineKeyboard(buttons).reply_markup,
                });
                return;
    ...
    if (state === 'AWAITING_PRODUCT_NAME') {
        ...
        await ctx.reply(`✅ Product Name set: *${text}*\n\n*(Step 3/3)* Now reply with the *Product Description*:`, { parse_mode: 'Markdown' });
        return;
    }
    ...
    if (state === 'AWAITING_PRODUCT_DESCRIPTION') {
        ...
        await ctx.reply(`✅ Description saved!\n\n*(Final Step)* Reply with the *Price in USD* (e.g. \`12.50\` or \`0\`):`, { parse_mode: 'Markdown' });
        return;
    }
    ...
    if (state === 'AWAITING_PRODUCT_PRICE') {
        ...
        await ProductService.createVariant(product.id, 'Standard License', priceNum, DeliveryType.AUTOMATIC);
        ctx.session.adminState = undefined;
        ctx.session.adminData = undefined;
        const successMsg = `🎉 *Product Added Live to Store in Real-Time!*\n\n` +
