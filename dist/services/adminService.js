...
export class AdminService {
    static async getDashboardMetrics() {
        ...
        const [totalUsers, totalOrders, todaysOrders, paidOrders, todaysPaidOrders, pendingPayments, pendingDeliveries, lowStockVariants,] = await Promise.all([
            prisma.user.count(),
            prisma.order.count(),
    ...
    static async importBulkStock(variantId, rawTextLines, fileId) {
        let importedCount = 0;
    ...
    static async approvePayment(paymentId, adminNotes, bot) {
    ...
    static async rejectPayment(paymentId, adminNotes, bot) {
        const provider = new ManualPaymentProvider();
