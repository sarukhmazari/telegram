...
export class WalletPaymentProvider {
    providerName = 'WALLET';
    async createPayment(orderId, userId, amount, currency, idempotencyKey) {
        ...
        const payment = await prisma.$transaction(async (tx) => {
            ...
            await UserService.updateBalance(userId, -amount, TransactionType.PURCHASE, `Payment for Order #${orderId}`, createdPayment.id);
    ...
    async refundPayment(paymentId) {
        ...
        await prisma.$transaction(async (tx) => {
            await UserService.updateBalance(payment.userId, Number(payment.amount), TransactionType.REFUND, `Refund for Payment #${payment.id}`, payment.id);
